...
<header class="text-gray-900 body-font p-6">
  ...
  <h1 class="text-center font-extrabold text-[40px]">Продукты</h1>
      ...
      <?php foreach ($products as $product):?>
        <div class="p-4 md:w-1/3 max-h-12">
          <div class="h-full border-2 bg-[#392D88] border-black border-opacity-60 rounded-lg overflow-hidden">
            <img class="lg:h-48 md:h-36 w-full object-cover object-center" src="../assets/img/<?= $product['img_product'] ?>" alt="blog">
            <div class="p-6">
              <h2 class="tracking-widest text-xs title-font font-medium text-white mb-1 uppercase"><?= $product['type_product'] ?></h2>
              <h1 class="title-font text-lg font-medium text-white mb-3"><?= $product['name_product'] ?></h1>
              <h1 class="title-font text-lg font-medium text-white mb-3">₽ <?= $product['cost_product'] ?></h1>
              <p class="leading-relaxed mb-5 text-white text-ellipsis overflow-hidden whitespace-no-wrap h-12"><?= $product['desc_product'] ?></p>
              <div class="flex items-center flex-wrap justify-center">
                <a href="./product_details.php?id_product=<?= $product['id_product']?> class="bg-[#392D88] text-white font-semibold px-8 py-2 rounded-xl border-2 border-white hover:bg-white hover:text-[#392D88] transition-all duration-700">Подробнее
                </a>
              </div>
            </div>
          </div>
